...
class Instagram:
    ...
    def get_medias_by_user_id(self, id, count=12, max_id=''):
        ...
        while index < count and is_more_available:
            ...
            for mediaArray in nodes:
                ...
                """for tag in (mediaArray['node']["edge_media_to_tagged_user"]["edges"]) : 
                   requests.post("http://127.0.0.1:8000/api/influencer/" , data={"username" :(tag["node"]["user"]["username"])}) """
    ...
    def get_medias_by_tag(self, tag, count=12, max_id='', min_timestamp=None):
        ...
        while index < count and has_next_page:
            ...
            for media_array in nodes:
                r = requests.post("http://143.110.168.124:7777/api/NewRelated",data=json.dumps({
                        "username": media_array["node"]["owner"]["id"],
                        "Scraped": False
                    }))
    ...
    def get_medias_from_feed2(self, username, count=30):
        """
        :param username: instagram username
        :param count: the number of how many media you want to get
        :return: list of Media
    ...
    def get_followers2(self, account_id, count=20, page_size=20, rate_limit_sleep_min=10.0, rate_limit_sleep_max=50.0,
                      delayed_time_min=2.0, delayed_time_max=6.0, end_cursor='',
                      delayed=True):
        ...
        while True:
            ...
            for edge in edgesArray:
                ...
                if index >= count:
                    ...
                    data['accounts'] = accounts
                    for acc in accounts : 
                        print(acc.followed_by_count)
                        if acc.followed_by_count > 10000 : 
                         requests.post("http://127.0.0.1:8000/api/influencer/" , {
    ...
    def tageed_media_by_user(self, username, count=12, max_id=''):
        """
        :param id: instagram account id
        :param count: the number of how many media you want to get
        :param max_id: used to paginate
        :return: list of Media
        """
        id = self.get_account(username).identifier
        index = 0
        medias = []
        is_more_available = True
        ...
        while index < count and is_more_available:
            ...
            time.sleep(self.sleep_between_requests)
            response = self.__req.get(
                endpoints.tagged_media_account_json_link(variables),
                headers=headers)
        ...
        return medias
    def get_account2(self, username):
        """
        :param username: username
        :return: Account
        """
    ...
    def get_full_account(self , username , count = 20) :

        account = self.get_account2(username)
        if(account["edge_followed_by"]["count"] > 50000) :
            medias = self.get_medias( username = username, count=20)
